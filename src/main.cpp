...
uint256 hashGenesisBlock("0xb2a9ed938ae7c992e71505ea33ae20d0fa944c7ba75d0ee0be256ebe3f532c8f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20);
static CBigNum bnStartDifficulty(~uint256(0) >> 24);
...
int64 CTransaction::nMinTxFee = 0 * COIN;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 0 * COIN;
...
{
    // Crown: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 0.00000001 * COIN;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 0.5 * 60;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 0.5 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 0.5 * 2.8;
...
{
    // Crown: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1404926203;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1526192;

        if (fTestNet)
        {
        	block.nTime    = 1398880557;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x4cdaef2656412731671098ec6785a5448c8d92954fb7aa35951bb84b157f3845"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xa8, 0xae, 0xcd };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CrownMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CrownMiner(CWallet *pwallet)
{
    printf("CrownMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CrownMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CrownMiner, pwallet));
}
