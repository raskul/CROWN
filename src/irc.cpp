...
// Copyright (c) 2011-2012 Crown Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CrownTEST0\r");
            Send(hSocket, "WHO #CrownTEST0\r");
        } else {
            // randomly join #Crown00-#Crown99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Crown%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Crown%02d\r", channel_number).c_str());
        }
